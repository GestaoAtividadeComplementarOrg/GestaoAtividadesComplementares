...
public interface SolicitacaoValidacaoRepository extends JpaRepository<SolicitacaoValidacao, Long> {
    ...
    @Query("SELECT COUNT(s) > 0 FROM SolicitacaoValidacao s JOIN s.itens i WHERE i.atividadeId = :atividadeId AND s.status IN :statusList")
    boolean existsByAtividadeIdAndStatusIn(@Param("atividadeId") Long atividadeId, @Param("statusList") Collection<StatusSolicitacao> statusList);
}
